...
def create_enhanced_executive_summary(leads_df, calls_df, schedule_df, agent_perf_df):
    ...
    <div class="dashboard-header">
        <h1 style="color: #f59e0b; margin: 0; font-size: 2.5rem;">ðŸš€ Executive CRM Dashboard</h1>
    ...
    with col1:
        ...
        st.markdown(f'<small style="color: #68d391;">ðŸ”¥ {hot_leads} Hot Leads</small>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(f'<small style="color: #68d391;">ðŸ’° ${expected_revenue:,.0f} Expected</small>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        st.markdown(f'<small style="color: #90cdf4;">ðŸ“ž {total_calls} Total Calls</small>', unsafe_allow_html=True)
    ...
    with col4:
        ...
        st.markdown(f'<small style="color: #68d391;">â­ {high_engagement} High Engagement</small>', unsafe_allow_html=True)
    ...
    with col5:
        ...
        st.markdown(f'<small style="color: #68d391;">ðŸŽ¯ {predicted_conversions} Likely Converts</small>', unsafe_allow_html=True)

    # AI Insights and Alerts
    st.subheader("ðŸ¤– AI-Powered Business Insights")
...
def create_enhanced_lead_status_dashboard(leads_df):
    ...
    with col1:
        ...
        if 'LeadStageId' in leads_df.columns:
            ...
            fig_funnel.update_layout(
                ...
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='white', size=12)
            )
    ...
    with col2:
        if 'BehavioralSegment' in leads_df.columns:
            ...
            fig_segments = go.Figure(data=[go.Pie(labels=segment_counts.index.astype(str), values=segment_counts.values, hole=0.4,
                                                  marker_colors=[colors.get(seg, '#6b7280') for seg in segment_counts.index])])
            fig_segments.update_layout(title='Customer Behavioral Segmentation', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='white', size=12))
    ...
    with col1:
        if 'TemperatureTrend' in leads_df.columns:
            ...
            fig_temp = go.Figure(data=[go.Bar(x=temp_counts.index.astype(str), y=temp_counts.values, marker_color=['#10b981', '#f59e0b', '#ef4444', '#6b7280'])])
    ...
    with col2:
        if 'EngagementScore' in leads_df.columns:
            ...
            fig_engagement.update_layout(title='Engagement Score Distribution', xaxis_title='Engagement Score', yaxis_title='Number of Leads', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='white'))
    ...
    with col3:
        if 'LeadVelocity' in leads_df.columns:
            ...
            fig_velocity = go.Figure(go.Indicator(mode='gauge+number', value=float(avg_velocity) if not np.isnan(avg_velocity) else 0.0,
    ...
    rev  = pd.to_numeric(leads_df.get('ExpectedRevenue', leads_df.get('RevenuePotential', pd.Series(*len(idx), index=idx))), errors='coerce').fillna(0)
    ...
    table = pd.DataFrame({'Lead': leads_df.get('FullName', pd.Series([f'Lead {i+1}' for i in range(len(idx))], index=idx)),
                          'Company': leads_df.get('Company', pd.Series(['-']*len(idx), index=idx)),
                          'Country': leads_df.get('Country', pd.Series(['-']*len(idx), index=idx)),
                          'QualityScore': quality.round(3), 'ConvProb': conv.fillna(0.0).round(3), 'Revenue': rev})
    top = table.sort_values(['QualityScore','Revenue'], ascending=[False,False]).head(10)

    st.markdown('**Top 10 Opportunities**')
    st.dataframe(top.style.format({'QualityScore': '{:.2f}', 'ConvProb': '{:.1%}', 'Revenue': '${:,.0f}'}), use_container_width=True, height=360)
    ...
    with g1:
        ...
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white')); st.plotly_chart(fig, use_container_width=True)
    with g2:
    ...
    st.dataframe(act_df.head(15).style.format({'Confidence':'{:.0%}','Revenue':'${:,.0f}','Buy':'{:.0%}','Churn':'{:.0%}','Upgrade':'{:.0%}'}), use_container_width=True, height=420)

def create_enhanced_call_activity_dashboard(calls_df):
    """Enhanced AI Call Activity Dashboard with pattern analysis"""
    st.subheader("ðŸ¤– Advanced Call Analytics & AI Insights")
    ...
    st.subheader("ðŸ“Š Call Pattern Intelligence")
    ...
    if 'Sentiment' in calls_df.columns:
        st.subheader("ðŸ˜Š Call Sentiment Analysis")
    ...
    if 'CallOutcome' in calls_df.columns:
        st.subheader("ðŸŽ¯ Call Outcome Analysis")
        ...
        with col2:
            st.markdown("**ðŸŽ¯ Outcome Insights:**")
    ...
    st.subheader("ðŸ¤– AI Call Strategy Recommendations")
    ...
    with col2:
        st.markdown("""
        <div class="prediction-box">
            <h4>ðŸ“ž Call Volume Prediction</h4>
...
def create_enhanced_task_dashboard(schedule_df):
    ...
    st.subheader("ðŸ“… Advanced Task Management & SLA Tracking")
    ...
    st.subheader("ðŸ“Š SLA Compliance & Performance")
    ...
    if 'SLAStatus' in schedule_df.columns:
        ...
        with col2:
            ...
            st.markdown("**ðŸš¨ SLA Alert Summary:**")
    ...
    st.subheader("ðŸ“ˆ Task Timeline & Workload Analysis")
    ...
    st.subheader("âš¡ Productivity & Efficiency Analytics")
    ...
    with col1:
        if 'EstimatedEffortHours' in schedule_df.columns:
            ...
            <div class="performance-card">
                <h4>â±ï¸ Total Effort Required</h4>
    ...
    st.subheader("ðŸŽ¯ AI Task Optimization Recommendations")
    ...
    with col1:
        st.markdown("""
        <div class="ai-recommendation">
            <h4>ðŸ¤– Workload Optimization</h4>
            <ul>
    ...
    with col2:
        st.markdown("""
        <div class="prediction-box">
            <h4>ðŸ“ˆ Completion Prediction</h4>
...
def create_enhanced_agent_dashboard(agent_perf_df, schedule_df):
    ...
    st.subheader("ðŸ‘¥ Advanced Agent Performance Analytics")
    ...
    st.subheader("ðŸŒŸ Individual Agent Scorecards")
    
    for _, agent in agent_perf_df.iterrows():
        with st.expander(f"ðŸ“Š {agent.get('AgentName', 'Unknown Agent')} - {agent.get('Role', 'Role Unknown')}"):
            col1, col2, col3, col4 = st.columns(4)
    ...
    st.subheader("ðŸ“Š Agent Performance Comparison")
    ...
    st.subheader("ðŸ¤– AI Agent Performance Insights")
...
def create_enhanced_conversion_dashboard(leads_df):
    ...
    st.subheader("ðŸ’¼ Advanced Conversion & Revenue Intelligence")
    ...
    # --- REVENUE BY SEGMENTS ---
    st.subheader("ðŸ’° Revenue Attribution Analysis")
    col1, col2 = st.columns(2)
    ...
    st.subheader("ðŸŽ¯ Conversion Probability Intelligence")
    c1, c2 = st.columns(2)
    ...
    st.subheader("ðŸ’Ž High-Value Conversion Opportunities")
    ...
    st.subheader("ðŸ“ˆ AI Revenue Forecasting")
    f1, f2, f3 = st.columns(3)
    with f1:
        forecast_30d = expected_revenue * 0.35
        st.markdown(f"""
        <div class='prediction-box'>
            <h4>ðŸ“… 30-Day Forecast</h4>
...
def create_enhanced_geographic_dashboard(leads_df):
    ...
    st.subheader("ðŸŒ Global Market Intelligence Dashboard")
    ...
    with c2:
        ...
        st.caption(f"ðŸ’° ${top_expected:,.0f} expected")
    with c3:
    ...
    # Charts row 2
    st.subheader("ðŸ“Š Market Intelligence Matrix")
    c1, c2 = st.columns(2)
...
def create_advanced_ai_insights_dashboard(leads_df, calls_df, schedule_df, agent_perf_df):
    ...
    st.subheader("ðŸ§  Advanced AI/ML Intelligence Center")
    ...
    # Critical AI Alerts
    st.subheader("ðŸš¨ AI-Powered Business Alerts")
    
    col1, col2 = st.columns(2)
    
    with col1:
        # High-priority alerts
        st.markdown("**âš¡ Immediate Action Required**")
        
        if 'ChurnRisk' in leads_df.columns:
            high_churn_leads = leads_df[leads_df['ChurnRisk'] > 0.8]
            st.markdown(f"""
            <div class="insight-card alert-high">
                <h4>ðŸ”¥ Churn Risk Alert</h4>
        ...
        if 'ConversionProbability' in leads_df.columns:
            ...
            st.markdown(f"""
            <div class="ai-recommendation">
                <h4>ðŸŽ¯ Hot Conversion Opportunities</h4>
    ...
    with col2:
        ...
        st.markdown("**ðŸ“Š Performance Optimization**")
        
        st.markdown("""
        <div class="prediction-box">
            <h4>âš¡ Call Optimization</h4>
    ...
    with col1:
        ...
        st.markdown("""
        <div class="prediction-box">
            <h4>ðŸ’° Revenue Forecast</h4>
            <p><strong>Next 7 Days:</strong> $342K (Â±$28K)</p>
            <p><strong>Next 30 Days:</strong> $1.2M (Â±$95K)</p>
            <p><strong>Next Quarter:</strong> $3.8M (Â±$310K)</p>
    ...
    with col2:
        ...
        if 'BehavioralSegment' in leads_df.columns:
            ...
            st.markdown(f"""
            <div class="ai-recommendation">
                <h4>ðŸ“ˆ Segment Evolution</h4>
                <p><strong>Champions:</strong> {segment_counts.get('Champions', 0)} â†’ {segment_counts.get('Champions', 0) + 3} (+3)</p>
                <p><strong>At Risk:</strong> {segment_counts.get('At Risk', 0)} â†’ {max(0, segment_counts.get('At Risk', 0) - 5)} (-5)</p>
    ...
    st.subheader("ðŸ” ML Model Insights & Feature Analysis")
    ...
    st.subheader("ðŸ’¡ AI-Powered Strategic Recommendations")
    ...
    st.subheader("âš¡ Real-time AI System Monitoring")
    ...
    with col1:
        st.markdown("""
        <div class="performance-card">
            <h4>ðŸ”„ Data Pipeline</h4>
            <h2 style="color: #10b981;">âœ“ Healthy</h2>
    ...
    with col2:
        ...
        <div class="performance-card">
            ...
            <h2 style="color: #10b981;">âœ“ Active</h2>
    ...
    with col3:
        ...
        <div class="performance-card">
            ...
            <h2 style="color: #f59e0b;">âš  94.2%</h2>
...
def main():
    ...
    st.sidebar.markdown("## ðŸŽ›ï¸ Advanced Analytics Filters")
    ...
    if 'BehavioralSegment' in leads_df.columns:
        ...
        selected_segments = st.sidebar.multiselect(
            "ðŸŽ¯ Behavioral Segments",
            segment_options,
            default=['All Segments']
        )
    
    # Real-time dashboard toggle
    auto_refresh = st.sidebar.checkbox("ðŸ”„ Auto-refresh (30 seconds)", value=False)
    ...
    if st.sidebar.button("ðŸ“‘ Generate Executive Report"):
        st.sidebar.success("Report generated! Check downloads.")
    
    if st.sidebar.button("ðŸ“ˆ Export Charts"):
    ...
    with col1:
        st.markdown("**ðŸ”„ Last Updated:** 2 minutes ago")
    
    with col2:
        st.markdown("**ðŸ“Š Data Status:** All systems operational")
    
    with col3:
        st.markdown("**ðŸ¤– AI Models:** 4 active, 87% avg accuracy")
    ...
    <div style="text-align: center; padding: 20px; color: #6b7280; font-size: 0.9rem; background: linear-gradient(90deg, #1a202c 0%, #2d3748 100%); border-radius: 10px; margin-top: 20px;">
        ðŸš€ Executive CRM Dashboard | Real Estate Analytics | Powered by Advanced AI/ML Intelligence<br>
