...
def create_enhanced_executive_summary(leads_df, calls_df, schedule_df, agent_perf_df):
    ...
    with c2:
        pipeline = float(leads_df.get('RevenuePotential', pd.Series()).sum())
        expected = float(leads_df.get('ExpectedRevenue', pd.Series()).sum())
    ...
    with c4:
        avg_eng = float(leads_df.get('EngagementScore', pd.Series()).mean())
        hi_eng = int((leads_df.get('EngagementScore', pd.Series())>80).sum())
        st.markdown(create_metric_card("Avg Engagement", avg_eng, 5.1), unsafe_allow_html=True)
        st.caption(f"⭐ {hi_eng} High Engagement")
    with c5:
        avg_conv = float(leads_df.get('ConversionProbability', pd.Series()).mean()*100)
        likely = int((leads_df.get('ConversionProbability', pd.Series())>0.7).sum())
...
# ---------- SAFE PREP FOR TASKS (fix for IndexingError) ----------
def _prepare_schedule(schedule_df: pd.DataFrame) -> tuple[pd.Series, str]:
    ...
    if 'DaysUntilDue' in schedule_df.columns:
        ...
        if days_to_due.shape != len(schedule_df) or days_to_due.isna().all():
...
def create_enhanced_task_dashboard(schedule_df):
    ...
    if status_col is not None:
        if status_col == 'TaskStatusId':
            ...
            completed_mask = schedule_df[status_col].isin([asset:1])    # Completed
        else:
    ...
    # SAFE: length‑aligned mask instead of get(..., pd.Series())
...
def create_enhanced_conversion_dashboard(leads_df):
    ...
    total_leads = len(leads_df)
    converted = int((leads_df.get('LeadStageId', pd.Series())==4).sum())
    pipeline = float(leads_df.get('RevenuePotential', pd.Series()).sum())
    expected = float(leads_df.get('ExpectedRevenue', pd.Series()).sum())
